...
computeCorr = function(matrix1, matrix2, scaleUp = TRUE) {
  stopifnot(nrow(matrix1) == nrow(matrix2))
  scaledCorrelations <- cor(matrix1, matrix2, use = "pairwise.complete.obs")
  if (scaleUp) {
    numNonMissingPairs <- Matrix::crossprod(!is.na(matrix1), !is.na(matrix2))
    scaledCorrelations %<>%
      multiply_by(numNonMissingPairs - 1)
  }
  scaledCorrelations
}
...
### If cor = TRUE use abs(correlation) as the metric; else if fisher = TRUE use the Fisher exact, otherwise chi-squared log p-value.
computeBestPValues = function(matrix1, matrix2, fisher = TRUE, cor = FALSE) {
  M <- nrow(matrix1)
  ...
  if (cor) {
    ...
    for (ind1 in 1:N1) {
      ...
      bestValue    <- selectorFun(curPVals, na.rm = TRUE)
      bestPs[ind1] <- bestValue
      bestps[ind1] <- coln[which(near(curPVals, bestValue, tol = .Machine$double.eps^0.5 * M))[1]] ### improvement: fixed tol
    }
  }
...
### The weights of agreement (scoreAgree, default 1) and disagreement (scoreDisagree, default -1) can be additionally specified.
computeAgreements = function(matrix1, matrix2, scoreAgree = 1L, scoreDisagree = -1L) {
  stopifnot(nrow(matrix1) == nrow(matrix2))
  N1 <- ncol(matrix1)
  N2 <- ncol(matrix2)
  if (N2 < N1) {
  ...
  for (ind in 1:N1) {
    ...
    output[ind,] <- scoreAgree * colSums(agree, na.rm = TRUE) + scoreDisagree * colSums(disagree, na.rm = TRUE)
  }
  output
}
...
### If keepDuplicates = FALSE, identical genotypes are pruned; otherwise, they are kept; identical phenotypes are always kept.
prepareMatrices = function(inputTab, numSNPs, complement, keepDuplicates = TRUE) {
  IDs <- inputTab %>% 
    pull("ID")
  inputTab %<>% 
    select(-ID) %>%
...
### If columnsOnly = TRUE, only operates the reductions on the columns (phenotypes), not the rows (patients), as is used for LCP
reducePhenotypesAndObjectives = function(phenotypes, objVectors, columnsOnly = FALSE, genotypes = NULL) {
...
### NB: the extreme value is in log-space if type != "LCP", and in real space otherwise
prepareExtremeValues = function(extremeValue, type, singleValues, singleBestRatio, useTighterBound) {
  ...
  if (!is.null(extremeValue)) {
    if (!is.na(singleBestRatio)) {
      ...
      if (type == "LCP") {
        altBounds %<>%
          multiply_by(singleBestRatio)
      } else {
        altBounds %<>%
          subtract(log(singleBestRatio))
      }
      altFunction <- ifelse((useTighterBound && type == "LCP") || (!useTighterBound && !(type == "LCP")), pmax, pmin)
      extremeValues %<>%
...
prepareBoundValues = function(extremeValues, type, objective, ns, sumYs, coeffsTrue, coeffsAgree) {
  startBound <- NA
  if (any(is.null(extremeValues)) && type != "LCP") {
  ...
  if (type == "CNF" && objective == "agreement") {
    boundValues %<>%
      add(ns * coeffsTrue + sumYs * coeffsAgree)
  }
  boundValues
}
